...
class Tbz_WC_Paylot_Gateway extends WC_Payment_Gateway {
    ...
    */
    public function admin_options() {
        ...
        <h4>Optional: To avoid situations where bad network makes it impossible to verify transactions, set your webhook URL <a href="https://dashboard.Paylot.co/#/settings/developer" target="_blank" rel="noopener noreferrer">here</a> to the URL below<strong style="color: red"><pre><code><?php echo WC()->api_request_url( 'Tbz_WC_Paylot_Webhook' ); ?></code></pre></strong></h4>
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'testmode' => array(
				...
				'type'        => 'checkbox',
				'description' => 'Test mode enables you to test payments before going live. <br />Once the LIVE MODE is enabled on your Paylot account uncheck this.',
				'default'     => 'yes',
				'desc_tip'    => true
			),
			'payment_page' => array(
	...
	public function verify_Paylot_transaction() {
		...
		if ( isset( $_REQUEST['Paylot_txnref'] ) ){
			...
			$Paylot_url = 'https://api.Paylot.co/transaction/verify/' . $_REQUEST['Paylot_txnref'];
	        ...
	        if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
				...
				if ( 'success' == $Paylot_response->data->status ) {
					...
					$order_details 		= explode( '_', $Paylot_response->data->reference );
