...
class pyl_WC_Paylot_Gateway extends WC_Payment_Gateway_CC {
	...
	*/
	...
	public function is_valid_for_use() {
		...
		if ( ! in_array( get_woocommerce_currency(), apply_filters( 'woocommerce_paylot_supported_currencies', array( 'NGN', 'USD', 'GBP', 'GHS' ) ) ) ) {

			$this->msg = 'paylot does not support your store currency. Kindly set it to either NGN (&#8358), GHS (&#x20b5;), USD (&#36;) or GBP (&#163;) <a href="' . admin_url( 'admin.php?page=wc-settings&tab=general' ) . '">here</a>';
    ...
    */
    public function admin_options() {
        ...
        <h4>Optional: To avoid situations where bad network makes it impossible to verify transactions, set your webhook URL <a href="https://demo.paylot.co/dashboard" target="_blank" rel="noopener noreferrer">here</a> to the URL below<strong style="color: red"><pre><code><?php echo WC()->api_request_url( 'pyl_WC_paylot_Webhook' ); ?></code></pre></strong></h4>
	...
	public function payment_scripts() {
		...
		wp_enqueue_script( 'paylot', 'https://js.paylot.co/v1/inline.min.js', array( 'jquery' ), WC_PAYLOT_VERSION, false );

		wp_enqueue_script( 'wc_paylot', plugins_url( 'assets/js/paylot.js', WC_PAYLOT_MAIN_FILE ), array( 'jquery', 'paylot' ), WC_PAYLOT_VERSION, false );
	...
	public function admin_scripts() {
		...
		wp_enqueue_script( 'wc_paylot_admin', plugins_url( 'assets/js/paylot-admin' . $suffix . '.js', WC_PAYLOT_MAIN_FILE ), array(), WC_PAYLOT_VERSION, true );
	...
	public function process_payment( $order_id ) {
		...
		if ( isset( $_POST['wc-' . $this->id . '-payment-token'] ) && 'new' !== $_POST['wc-' . $this->id . '-payment-token'] ) {
			...
			if ( is_user_logged_in() && isset( $_POST['wc-'. $this->id .'-new-payment-method'] ) && true === (bool) $_POST['wc-'. $this->id .'-new-payment-method'] && $this->saved_cards ) {
				...
				update_post_meta( $order_id, '_wc_paylot_save_card', true );
	...
	public function receipt_page( $order_id ) {
		...
		if( 'embed' == $this->payment_page ) {
			...
			echo '<div id="paylot_form"><form id="order_review" method="post" action="'. WC()->api_request_url( 'pyl_WC_Paylot_Gateway' ) .'"></form>
		...
		if ( isset( $_REQUEST['paylot_txnref'] ) ){
	        ...
	        if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
				...
				if ( true == $paylot_response->sent ) {
	        		...
	        		$amount_paid        = $paylot_response->data->amount / 100;

	        		$paylot_ref       = $paylot_response->data->reference;

					$payment_currency   = $paylot_response->data->currency;
	 ...
	 */
	public function save_subscription_payment_token( $order_id, $paylot_response ) {

		echo"<script>alert('payment token not saved')</script>";
